...
BEGIN
  IF NOT EXISTS (
    ...
    ALTER TABLE public.courses ADD COLUMN difficulty_level INTEGER DEFAULT 1 CHECK (difficulty_level BETWEEN 1 AND 5);
  END IF;
END $$;
...
-- Create enrollments table if it doesn't exist (for M:N relationship)
...
DROP POLICY IF EXISTS "Published course modules are viewable by everyone" ON public.course_modules;
...
CREATE INDEX IF NOT EXISTS idx_course_modules_course_module ON public.course_modules(course_id, module_number);

COMMENT ON TABLE public.enrollments IS 'M:N relationship between students and courses';
COMMENT ON TABLE public.course_modules IS 'Modules generated by AI for each course';
