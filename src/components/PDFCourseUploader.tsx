  ...
  const handleUpload = async () => {
    ...
    try {
      ...
      const courseInsert = {
        ...
        description: preferences ? preferences : `AI-generated course from ${file.name}`,
        category: 'AI Generated',
        difficulty_level: 2,
        is_published: true, // Auto-publish PDF courses so students can see them
        metadata: preferences ? { learning_preferences: preferences } : null
      }
      ...
      // Step 2: Process PDF and generate modules (no hardcoded limits!)
      const result = await processPDFAndCreateCourse(file, course.id, (msg) => setProgress(msg))
      ...
      if (result.success && result.modules) {
        setResult({
          ...
          cost: '$2-5',
          learningObjectives: result.modules.slice(0, 3).map(m => m.title)
        })
      ...
      setError(err.message || 'Failed to process PDF. Check console for details.')
    } finally {
      setUploading(false)
    }
  }
