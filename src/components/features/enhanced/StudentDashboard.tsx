...
import { ProgressChart, MultiLineChart } from '@/components/charts/ProgressChart';
...
const ProgressTab = ({ weeklyProgress, achievements }: any) => {
  const chartData = weeklyProgress.map((day: any) => ({
    day: day.day,
    hours: day.hours,
    lessons: day.lessons,
  ...
  return (
    <div className="space-y-8">
      ...
      <MultiLineChart
        ...
        lines={[
          { dataKey: 'hours', color: 'hsl(var(--fl-primary))', name: 'Hours' },
          { dataKey: 'lessons', color: 'hsl(var(--fl-secondary))', name: 'Lessons' },
          { dataKey: 'accuracy', color: 'hsl(var(--fl-success))', name: 'Accuracy %' }
        ]}
      />
