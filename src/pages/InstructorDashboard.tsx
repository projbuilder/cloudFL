...
import { LogOut, User as UserIcon, Plus, BookOpen, Users, BarChart, Edit, Eye, Trash2, X, Upload, Sparkles, MessageSquare } from 'lucide-react'
...
export default function InstructorDashboard() {
  ...
  const [activeTab, setActiveTab] = useState<'courses' | 'analytics' | 'discussions'>('courses')
  ...
  async function handleCreateCourse() {
    ...
    try {
      await courseService.createCourse({
        ...
        is_published: true, // Auto-publish new courses so students can see them
        category: 'General',
        difficulty_level: newCourse.level === 'Beginner' ? 1 : newCourse.level === 'Intermediate' ? 2 : 3
      })
  ...
  async function handleGenerateQuiz(courseId: string) {
    try {
      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
                    ...
                    <div className="p-6">
                      ...
                      <div className="space-y-2">
                        <Link
                          ...
                          className="w-full py-2 bg-gradient-to-r from-fl-primary to-fl-secondary text-white rounded-lg hover:shadow-glow transition-all text-sm flex items-center justify-center gap-2"
                        >
                        ...
                        <div className="flex gap-2">
                          <button 
                            ...
                            className="flex-1 py-2 bg-gradient-to-r from-purple-500 to-pink-500 text-white rounded-lg hover:shadow-glow transition-all text-sm flex items-center justify-center gap-2"
                          >
                            <Sparkles className="w-4 h-4" />
                            AI Quiz
                          </button>
        ...
        {activeTab === 'analytics' && user && (
          <div>
            ...
            <InstructorAnalyticsDashboard instructorId={user.id} />
          </div>
        )}

        {activeTab === 'discussions' && (
          <div>
            <h3 className="text-2xl font-bold mb-6">Course Discussions</h3>
            <div className="glass-card p-8 rounded-xl text-center">
              <MessageSquare className="w-16 h-16 text-muted-foreground mx-auto mb-4 opacity-50" />
              <h4 className="text-xl font-semibold mb-2">Discussion Management</h4>
              <p className="text-muted-foreground mb-4">
                View and moderate student discussions across all your course modules
              </p>
              <p className="text-sm text-muted-foreground">
                Students can access discussions from the course page → Discussions tab
              </p>
            </div>
          </div>
        )}
